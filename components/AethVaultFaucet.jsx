...
export default function AethVaultFaucet({ account, walletProvider, faucetAddress, showToast, TARGET_CHAIN_NAME, t }) {
  const [isClaiming, setIsClaiming] = useState(false);
  const [claimAmount, setClaimAmount] = useState(0);
  const [waitTime, setWaitTime] = useState(0);
  const [nextAccess, setNextAccess] = useState(0);
  ...
  const fetchFaucetData = useCallback(async () => {
    ...
    try {
      ...
      const [amount, wait, next, pausedState] = await Promise.all([
        contract.claimAmount(),
        contract.WAIT_TIME(),
        contract.nextAccessTime(account || "0x0000000000000000000000000000000000000000"),
        contract.paused()
      ]);

      setClaimAmount(parseFloat(ethers.formatUnits(amount, 18)));
      setWaitTime(Number(wait));
      setNextAccess(Number(next));
  ...
  const handleClaim = async () => {
    if (!account) return showToast(t?.connectWalletFirst || "Hubungkan dompet terlebih dahulu", "error");
    if (isPaused) return showToast(t?.faucetErrPaused || "Faucet sedang dihentikan sementara (Paused)", "error");
    if (timeLeft > 0) return showToast(t?.faucetErrCooldown || "Anda masih dalam masa cooldown!", "error");
    ...
    try {
      ...
      showToast(t?.faucetMsgClaiming || "Memproses klaim token...", "info");
      const tx = await contract.claim();
      await tx.wait();

      const successMsg = t?.faucetMsgSuccess ? t.faucetMsgSuccess.replace('{amount}', claimAmount) : `Berhasil! ${claimAmount} AETH telah dikirim ke dompet Anda.`;
      showToast(successMsg, "success");
      await fetchFaucetData(); 
    } catch (error) {
      showToast((t?.faucetMsgFail || "Gagal klaim: ") + (error.reason || error.message || "Transaksi ditolak"), "error");
    } finally {
      setIsClaiming(false);
    }
  };
  ...
  const formatTime = (seconds) => {
    if (seconds <= 0) return t?.faucetReady || "Ready to Claim";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="bg-[#05030F] border border-blue-500/30 px-4 py-2.5 rounded-xl flex items-center gap-3 shadow-[0_0_15px_rgba(59,130,246,0.15)]">
          ...
          <div className="flex flex-col">
            <span className="text-[9px] text-neutral-400 uppercase tracking-widest">{t?.faucetAllocation || "Alokasi per Dompet"}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
        <div className="bg-[#05030F] border border-neutral-800 p-5 sm:p-6 rounded-xl sm:rounded-2xl space-y-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-bold text-neutral-300">{t?.faucetSecurity || "Status Keamanan"}</span>
          </div>
          <div className="flex justify-between items-center border-b border-neutral-900 pb-3">
            <span className="text-xs text-neutral-500">{t?.faucetWallet || "Alamat Dompet"}</span>
            <span className="text-xs font-mono text-cyan-300 flex items-center gap-1.5">
              <Wallet className="w-3 h-3" />
              {account ? `${account.substring(0,6)}...${account.substring(account.length-4)}` : (t?.faucetNotConnected || 'Belum Terhubung')}
            </span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-xs text-neutral-500">{t?.faucetCooldown || "Waktu Tunggu (Cooldown)"}</span>
        ...
        <div className="bg-[#05030F] border border-neutral-800 p-5 sm:p-6 rounded-xl sm:rounded-2xl flex flex-col justify-center items-center text-center">
          {isPaused ? (
            <div className="text-red-400 font-bold text-sm bg-red-500/10 px-4 py-3 rounded-xl border border-red-500/20 w-full">
              {t?.faucetPausedAlert || "Faucet Sedang Dinonaktifkan (Paused)"}
            </div>
          ) : (
            <>
              <button
              ...
              >
                {isClaiming ? (
                  <><Loader2 className="w-4 h-4 animate-spin" /> {t?.faucetProcessing || "Memproses Transaksi..."}</>
                ) : timeLeft > 0 ? (
                  <><Timer className="w-4 h-4" /> {t?.faucetCooldownActive || "Cooldown Aktif"}</>
                ) : (
                  <>{t?.faucetClaimBtn ? t.faucetClaimBtn.replace('{amount}', claimAmount) : `Klaim ${claimAmount} AETH`} <ArrowRight className="w-4 h-4" /></>
                )}
              </button>
              <p className="text-[10px] text-neutral-500 mt-4">
                {t?.faucetGasNote || "*Dibutuhkan sedikit tBNB untuk membayar biaya gas transaksi."}
              </p>
            </>
          )}
        </div>
