...
// =========================================================
// 🚀 GAYA WEB3 FUTURISTIK: FINAL PRESISI & UTUH
// =========================================================
const CertificateTemplate = React.forwardRef(({ proofData, tDash, tHop, formatAddress, categoryConfig, AETHER_LOGO = '/logo.png' }, ref) => {
  ...
  return (
    <div id="cert-export-node" ref={ref} className="w-[890px] h-[660px] bg-[#0A0714] text-gray-200 rounded-2xl p-6 pb-4 relative overflow-hidden font-sans border border-amber-500/30 mx-auto flex flex-col justify-between shrink-0 shadow-[0_0_50px_rgba(139,92,246,0.15)]" style={{ background: 'radial-gradient(ellipse at 20% 0%, #11112d 0%, #090718 42%, #03020a 100%)', borderImage: 'linear-gradient(135deg, #06b6d4, #8b5cf6, #f59e0b, #8b5cf6, #06b6d4) 1', borderWidth: '2px', borderStyle: 'solid' }}>
      ...
      {/* HEADER */}
      <div className="flex justify-between items-center relative z-10 pb-2 border-b border-neutral-800/60 shrink-0">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl flex items-center justify-center border border-cyan-400/40 bg-black/50 shadow-[0_0_20px_rgba(6,182,212,0.25)]"><img src={AETHER_LOGO} alt="AetherVault" style={{ width: '32px', height: '32px' }} /></div>
          <div><div className="font-display font-black text-lg tracking-[0.18em] text-white">AETHER<span className="text-amber-400">VAULT</span></div><p className="text-[7px] tracking-[0.3em] text-cyan-300/70 uppercase font-mono mt-1">Trustless • Verified • Timeless</p></div>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1.5 bg-neutral-950/80 px-3 py-1 rounded-full border border-green-500/30"><div className="w-2 h-2 bg-green-400 rounded-full animate-pulse" /><span className="text-[8px] font-bold text-green-300 uppercase tracking-widest font-mono">VERIFIED ON-CHAIN</span></div>
          <div className="bg-neutral-950/80 px-3 py-1 rounded-full border border-amber-500/30 flex items-center gap-1.5"><Globe className="w-3 h-3 text-amber-400" /><span className="text-[8px] font-bold text-amber-300 uppercase tracking-widest font-mono">BNB CHAIN</span></div>
        </div>
      </div>

      {/* TITLE */}
      <div className="text-center relative z-10 shrink-0 my-1">
        <div className="inline-flex items-center gap-2 mb-0.5">
          <Sparkles className="w-3.5 h-3.5 text-amber-400" />
          <span className="text-[10px] tracking-[0.4em] text-neutral-400 uppercase font-mono font-bold">Non-Fungible Token Certificate</span>
          <Sparkles className="w-3.5 h-3.5 text-amber-400" />
        </div>
        <h2 className="text-3xl font-black tracking-[0.15em] font-display text-transparent bg-clip-text bg-gradient-to-r from-cyan-200 via-white to-amber-200">CERTIFICATE OF AUTHENTICITY</h2>
      </div>
          ...
          <div className="flex-1 bg-[#05030F]/60 border border-neutral-800/60 rounded-2xl p-3.5 shadow-inner backdrop-blur-md flex flex-col justify-between font-mono">
            <div className="flex justify-between items-start mb-1">
              <div className="flex-1 pr-3">
                ...
                <p className="text-white font-bold text-sm sm:text-base line-clamp-1">{proofData?.title || 'Aether Proof™'}</p>
              </div>
              <div className="text-right shrink-0">
                <p className="text-[8px] uppercase tracking-[0.3em] text-neutral-500 mb-0.5 font-bold">Certificate ID</p>
                <p className="text-cyan-400 font-bold text-xs sm:text-sm tracking-wider drop-shadow-[0_0_8px_rgba(6,182,212,0.5)]">
            ...
            <div className="mb-1 flex-1">
              <p className="text-[8px] uppercase tracking-[0.3em] text-neutral-500 mb-0.5 font-bold">Description</p>
              <p className="text-[10px] sm:text-xs text-neutral-300 leading-snug line-clamp-2 pr-4">
            ...
            <div className="flex justify-between items-center pt-2 border-t border-neutral-800/60">
              <div className="w-1/4 pr-2"><p className="text-[7px] uppercase tracking-[0.2em] text-neutral-500 mb-0.5 font-bold">Creator Name</p><p className="text-neutral-200 font-bold text-[10px] sm:text-xs truncate">{proofData?.creator || 'Unknown Creator'}</p></div>
              <div className="w-1/4 pr-2"><p className="text-[7px] uppercase tracking-[0.2em] text-neutral-500 mb-0.5 font-bold">Owner Wallet</p><p className="text-neutral-200 font-bold text-[10px] sm:text-xs truncate font-mono">{formatAddress(proofData?.wallet || '0x00...00')}</p></div>
              <div className="w-1/4 pr-2"><p className="text-[7px] uppercase tracking-[0.2em] text-neutral-500 mb-0.5 font-bold">Issued On</p><p className="text-neutral-200 font-bold text-[10px] sm:text-xs">{proofData?.date || new Date().toLocaleDateString()} • UTC</p></div>
              <div className="w-1/4"><p className="text-[7px] uppercase tracking-[0.2em] text-neutral-500 mb-0.5 font-bold">Blockchain</p><p className="text-neutral-200 font-bold text-[10px] sm:text-xs truncate">{proofData?.network || 'Binance Smart Chain'}</p></div>
            </div>
          </div>
        </div>

        {/* BARIS 2: METADATA */}
        <div className="bg-[#0a0a1a]/80 rounded-xl py-2 px-3.5 border border-cyan-500/20 shadow-lg flex items-center gap-3 w-full font-mono shrink-0">
          <div className="flex items-center gap-2 pr-3 border-r border-neutral-800/80 shrink-0">
          ...
          <div className="flex-1 flex justify-between items-center text-[10px] sm:text-xs">
            <div className="flex items-center gap-1.5 w-[15%]"><Hash className="w-3.5 h-3.5 text-neutral-500 shrink-0" /><div className="min-w-0"><p className="text-[7px] text-neutral-500 uppercase font-bold">Token ID</p><p className="text-white font-mono truncate font-bold">#{proofData?.tokenId || '0'}</p></div></div>
            <div className="flex items-center gap-1.5 w-[15%]"><Cpu className="w-3.5 h-3.5 text-neutral-500 shrink-0" /><div className="min-w-0"><p className="text-[7px] text-neutral-500 uppercase font-bold">Chain ID</p><p className="text-white font-mono truncate font-bold">97</p></div></div>
            <div className="flex items-center gap-1.5 w-[25%] pr-2"><FileDigit className="w-3.5 h-3.5 text-neutral-500 shrink-0" /><div className="min-w-0 w-full"><p className="text-[7px] text-neutral-500 uppercase font-bold">Contract</p><p className="text-cyan-300 font-mono text-[10px] truncate w-full font-bold">{formatAddress(proofData?.contract || AETHER_VAULT_ADDRESS)}</p></div></div>
            <div className="flex items-center gap-1.5 w-[45%]"><Code2 className="w-3.5 h-3.5 text-cyan-400 shrink-0" /><div className="min-w-0 w-full"><p className="text-[7px] text-neutral-500 uppercase font-bold">Tx Hash (SHA-256)</p><p className="text-cyan-300 font-mono text-[10px] truncate w-full font-bold">{proofData?.fileHash || 'Calculating...'}</p></div></div>
          </div>
        </div>

        {/* 🌟 BARIS 3: KODE SESUAI PERMINTAAN (Poin 2: 100% ON-CHAIN DI TENGAH PENDING MINT & BARCODE) */}
        <div className="flex gap-3.5 font-mono shrink-0 items-center">
          <div className="flex-1 bg-[#05030F]/80 border border-neutral-800 rounded-2xl p-3.5 flex items-center justify-between shadow-lg">
            <div>
              <p className="text-[8px] text-neutral-500 uppercase tracking-widest font-bold">Certificate Serial</p>
              <p className="text-sm sm:text-base font-bold mt-0.5 font-mono text-white">#{proofData?.id || proofData?.tokenId || 'PENDING-MINT'}</p>
            </div>
            
            {/* TULISAN 100% ON-CHAIN DI TENGAH */}
            <div className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-cyan-950/40 border border-cyan-500/30 text-cyan-300 text-[10px] font-bold uppercase tracking-widest shadow-[0_0_12px_rgba(6,182,212,0.2)]">
              <Shield className="w-3.5 h-3.5 text-cyan-400 animate-pulse" /> 100% On-Chain
            </div>

            <div className="relative av-badge-pulse" style={{ color: cat.color }}>
               <div className="flex items-center gap-1.5 px-4 py-2 rounded-lg border font-bold text-xs tracking-[0.15em] shadow-[0_0_15px_currentColor]" style={{ background: cat.bg, borderColor: cat.border, color: cat.color }}>
                 {CatIcon} <span>{cat.badgeLabel || cat.label?.toUpperCase() || 'AUTHENTIC'}</span>
               </div>
            </div>
          </div>

          <div className="w-[100px] bg-[#05030F]/80 border border-neutral-800/60 rounded-2xl p-2 flex items-center justify-center shadow-lg shrink-0">
            <div className="w-[74px] h-[74px] bg-white p-1 rounded-lg shadow-sm flex items-center justify-center">
              <QRCode value={proofData?.verifyUrl || "https://aethvault.xyz"} size={66} bgColor="#ffffff" fgColor="#0A0714" level="Q" />
            </div>
          </div>
        </div>
      ...
      {/* 🌟 FOOTER (Poin 1: Kiri Kanan Dirapatkan ke Siku, Poin 3: Animasi Logo Diaktifkan) */}
      <div className="relative z-10 pt-2 pb-1 border-t border-neutral-800/60 flex flex-row items-center justify-between px-1.5 shrink-0">
        ...
        <div className="text-right pr-0.5">
          ...
          <div className="flex items-center justify-end gap-1.5 mt-0.5">
            ...
            <span className="text-[8px] font-mono text-amber-300 bg-amber-500/10 px-2 py-0.5 rounded-full border border-amber-500/30 font-bold">BSC TESTNET</span>
          </div>
        </div>
      </div>
    </div>
...
// =========================================================
// MAIN COMPONENT: AETHER PROOF HUB (UTUH TANPA PANGKASAN)
// =========================================================
