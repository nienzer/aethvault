        ...
        <div className="lg:col-span-8 bg-[#0B0817] border border-neutral-900 rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-xl space-y-6">
          <div>
            <h3 className="font-display text-lg sm:text-xl font-bold text-white mb-1 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-cyan-400" /> Create New Capsule
            </h3>
            <p className="text-xs text-neutral-400">Secure your messages or files with military-grade encryption.</p>
          ...
          <form onSubmit={handleSeal} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-cyan-500 uppercase tracking-widest font-mono">Capsule Title</label>
              <input
                type="text"
                value={title}
            ...
            <div className="space-y-3">
              <label className="block text-[10px] font-bold text-cyan-500 uppercase tracking-widest font-mono">Security Tier</label>
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between border-t border-neutral-800/80 pt-3 w-full">
                        <span className="text-[10px] text-red-400 font-bold flex items-center gap-1 font-mono">
                          <Flame className="w-3.5 h-3.5" /> Burn {data.burn} AETH
                        </span>
                        {isSelected && (
            ...
            <div className="space-y-2 pt-2">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-bold text-cyan-500 uppercase tracking-widest font-mono flex items-center gap-1.5">
                  <UploadCloud className="w-4 h-4" /> File Attachment
                </label>
              ...
              <div className={`border-2 border-dashed rounded-2xl p-6 text-center transition-all ${isPermanentTier ? 'border-cyan-500/30 hover:border-cyan-500/60 bg-[#05030F]' : 'border-neutral-800 bg-[#0B0817] opacity-50 cursor-not-allowed'}`}>
                {!isPermanentTier ? (
                  ...
                  <div className="py-4">
                    ...
                    <p className="text-xs font-bold text-cyan-400">Encrypting & Estimating Storage...</p>
                  </div>
                ) : stagedUpload ? (
                  <div className="text-left space-y-3">
                    ...
                    <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-3">
                      <p className="text-xs text-purple-200 font-mono">
                        Estimated Arweave Cost: <span className="font-bold text-white">~{stagedUpload.estimatedCost} POL</span>
                      </p>
                    </div>
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-cyan-500 uppercase tracking-widest font-mono">Secret Message (Encrypted)</label>
              <textarea
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                placeholder="Type the message you want to secure..."
              ...
              <div className="text-right text-[11px] text-cyan-400 font-mono font-bold">
                Characters: {message.length} / {tiers[tier].maxLength}
              </div>
            </div>
            ...
            <div className="pt-2">
              {tier === 'legacy' ? (
                <div className="space-y-4 bg-red-950/10 border border-red-500/20 p-5 rounded-2xl">
                  <div>
                    <label className="block text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 font-mono">Inactivity Limit</label>
                    <select
                    ...
                    >
                      <option value="5">5 Years Inactivity</option>
                      <option value="10">10 Years Inactivity</option>
                      <option value="20">20 Years Inactivity</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-red-400 uppercase tracking-widest mb-2 font-mono">Heir Wallet Address</label>
                    <input
                      type="text"
                      value={heirAddress}
                      ...
                      required
                    />
                    <p className="text-[10px] text-neutral-500 mt-1.5 font-mono">The designated heir must register their public key in settings to claim.</p>
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  <label className="block text-[10px] font-bold text-cyan-500 uppercase tracking-widest font-mono">Unlock Date & Time</label>
            ...
            >
              ...
              {isSealing ? "Sealing Capsule on Blockchain..." : isWrongNetwork ? `Switch to ${TARGET_CHAIN_NAME}` : (isConnected ? "Seal Capsule" : "Connect Wallet to Seal")}
            </button>
          </form>
        </div>
