...
// - Provider URL mengarah ke BSC Testnet Testnet RPC
...
// - Pastikan user sudah punya testnet tBNB/POL di wallet-nya.
...
const IRYS_TIMEOUT_MS = 60000; // 60s untuk testnet
    ...
    const uploader = await withTimeout(
      WebUploader(WebEthereum).withAdapter(
        EthersV6Adapter(browserProvider, {
          ...
          providerUrl: "https://bsc-testnet-rpc.publicnode.com",
        })
      ),
      IRYS_TIMEOUT_MS,
...
export async function uploadToArweavePermanent(browserProvider, data, tags = []) {
  ...
  if (currentBalance < priceAtomic) {
    ...
    let lastErr = null;
    // Retry fund up to 3x untuk testnet
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        await withTimeout(
          uploader.fund(shortfall),
          IRYS_TIMEOUT_MS,
          `Pembayaran Irys (fund) - percobaan ${attempt}/3`
        );
        fundSuccess = true;
        break;
      } catch (err) {
        lastErr = err;
        console.warn(`Fund Irys gagal (percobaan ${attempt}/3):`, err?.message);
        if (attempt < 3) {
          await new Promise(r => setTimeout(r, 3000)); // tunggu 3s sebelum retry
        }
      }
    }
    if (!fundSuccess) {
      ...
      throw new Error(
        msg.includes("timeout")
          ? msg + " — Irys devnet mungkin lambat, coba lagi nanti."
          : "Gagal membayar biaya penyimpanan Arweave setelah 3x percobaan. Pastikan saldo testnet POL mencukupi dan coba lagi."
      );
    }
  }
